...
async function cartAdd(cartDetails) {
    logger.debug(`Inside cartservices`);
    try {
        const productExist = await cartDao.checkProductExists(cartDetails);
        console.log(productExist);
        if (productExist) {
            return cartMapper.responseMapping(cartConstants.CODE.INTRNLSRVR, cartConstants.MESSAGES.productExistsInCart)
        }
...
function removeProductFromCart(...args) {
    try {
        return cartDao.removeProductFromCart(...args).then((data) => {
            return cartMapper.responseMapping(cartConstants.CODE.Success, cartConstants.MESSAGES.Cart_Deleted)
        }).catch((err) => {
            return cartMapper.responseMapping(cartConstants.CODE.INTRNLSRVR, cartConstants.MESSAGES.internalServerError)
        })
