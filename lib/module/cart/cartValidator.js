...
const validateCart = function (req, res, next) {
    let cart_name = req.body.cart_name; 
    let status = req.body.status; 
    const errors = [];

    if (_.isEmpty(cart_name)) {
        errors.push({ fieldName: "cart_name", message: cartConstants.MESSAGES.CartNameCantEmpty });
    } else if (_.isEmpty(status)) {
        errors.push({ fieldName: "status", message: cartConstants.MESSAGES.statusCantEmpty });
    } else {
