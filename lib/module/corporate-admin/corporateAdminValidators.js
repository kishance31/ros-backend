...
const validateResetPassword = (req, res, next) => {
    let { resetToken, new_password } = req.body;

    const errors = [];

    if (_.isEmpty(resetToken)) {
        errors.push(
            userMapper.responseMapping(userConst.CODE.Unauthorized, userConst.MESSAGES.RESET_TOKEN_NOT_FOUND)
        );
    }
    
    if (_.isEmpty(new_password)) {
        errors.push(
            userMapper.responseMapping(userConst.CODE.Unauthorized, userConst.MESSAGES.PWD_CANT_EMPTY)
        );
    }
...
const validateRecurringPayment = (req, res, next) => {
    const { orderId, paypalDetails, invoiceNo, corporateId } = req.body;
    const errors = [];

    if (!orderId) {
        errors.push({ code: 500, message: "Order Id empty." });
    } else if (!invoiceNo) {
        errors.push({ code: 500, message: "invoice Id empty." });
    } else if (!corporateId) {
        errors.push({ code: 500, message: "corporate Id empty." });
    } else if (_.isEmpty(paypalDetails)) {
        errors.push({ code: 500, message: userConst.MESSAGES.validationError });
    }
...
function validateUpdateUser(req, res, next) {
    const { username, firstName, lastName, companyName, position, department, personalEmailId,
