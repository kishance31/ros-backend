...
function employeeOrdersEditById(id, data) {
	try {
		return userDao.updateEmpOrders(id, data).then((data) => {
			return userMapper.responseMappingData(corporateAdminConstants.CODE.Success, corporateAdminConstants.MESSAGES.Employee_Orders_Update, data)

		}).catch((err) => {
			return userMapper.responseMapping(corporateAdminConstants.CODE.INTRNLSRVR, corporateAdminConstants.MESSAGES.internalServerError)
...
function getEmployeeNames(id) {
	try {
		return userDao.getEmployeeNames(id).then((data) => {
			return userMapper.responseMappingData(corporateAdminConstants.CODE.Success, "Employee list success", data)
	...
	try {
		...
		if (result) {
			return userDao.confirmEmployeeOrders(orders, update).then((data) => {
				return userMapper.responseMapping(corporateAdminConstants.CODE.Success, "Order Payment Success.")
...
async function getInvoiceList(query, batch, limit) {
	try {
		return userDao.getInvoiceList(query, batch, limit).then((data) => {
			return userMapper.responseMappingList(corporateAdminConstants.CODE.Success, "Invoice list Success.", data)
		}).catch((err) => {
			console.log(err);
			return userMapper.responseMapping(corporateAdminConstants.CODE.INTRNLSRVR, corporateAdminConstants.MESSAGES.internalServerError)
		})
