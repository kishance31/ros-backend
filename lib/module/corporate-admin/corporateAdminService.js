...
function corporate_signup_mail(corpDetails) {
  try {
    ...
    // };
    let Intro =
      "Thank you for registering with us as your profile has been successfully sent for Verification!";
...
async function sendOrderToVendor(vendorDetails) {
  try {
    let Intro = `Here is the product details that need to dispatch ${vendorDetails.OrderDetails[0].productName}
    <p>Thanks</p`;
    const html = mailHandler.mailGenHTML({
    ...
    mailHandler.sendMail({
      ...
      subject: corporateAdminConstants.MESSAGES.Order_received,
      html,
    });
    return userMapper.responseMapping(
      corporateAdminConstants.CODE.Success,
