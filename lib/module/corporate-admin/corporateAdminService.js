...
async function sendOrderToVendor(vendorDetails) {
	try {
		...
		let orderTable = [
			"<table><thead><th>Product Code</th><th>Product Name</th><th>Product Image</th></thead><tbody>"
		]
		vendorDetails.orderDetails.products.forEach(order => {
			orderTable.push(`<tr><td>${order.product_code}</td><td>${order.product_name}</td><td><img src="${order.product_image[0]}" height="100" width="100" /></td></tr>`)
		})
		orderTable.push("</tbody></table>");
		Intro += orderTable.join("");

		Intro += "<br/><div style='display: flex;'><p>Customer Name:- </p> &nbsp;&nbsp;";
		Intro += "<p>" + vendorDetails.orderDetails.employeeDetails.firstName + " " + vendorDetails.orderDetails.employeeDetails.firstName + "</p> </div>";
		Intro += "<br/><p>Delivery address:- </p> <br />";
		Intro += `
