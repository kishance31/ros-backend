...
async function createFirstInvoice(orders, paypalDetails) {
    try {
        ...
        let invoiceNos = await appUtil.generateNumberNanoId();

        let invoiceDetails = {
            corporateId: ordersList[0].corporateId,
        ...
        ordersList.forEach((order, idx) => {
            ...
            invoiceDetails.employeeId.push(employeeId);
            invoiceDetails.orderId.push(orderId);
            invoiceDetails.orderDate.push(orderDate);
            invoiceDetails.firstTimeCost += firstTimeCost;
            invoiceDetails.recurringCost += recurringCost;
            invoiceDetails.totalOrderCost += totalOrderCost;
        })
