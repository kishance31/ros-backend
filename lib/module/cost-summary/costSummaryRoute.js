...
costSummaryRouter.route('/saveCostSummary').post([authMiddleware.autntctAdminTkn],(req, res) => {
    costSummaryFacade.costSummaryAdd(req.body, req.tokenPayload.userId)
        .then((result) => {
...
costSummaryRouter.route('/getCostSummaryById/:id').get([authMiddleware.autntctTkn],(req, res) => {
    const data = {
        _id: req.params.id
    };
    costSummaryFacade.getCostSummaryById(data)
        .then(result => {
            responseHandler.sendSuccess(res, result);
        })
        .catch(err => {
