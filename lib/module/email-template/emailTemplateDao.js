...
async function emailTemplateList(batch, limit) {
    // return await emailTemplateDao.find();
    return await emailTemplateDao.aggregate([
        {
            $facet: {
                list: [
                    { $match: {} },
...
async function updateEmailTemplate(data, _update) {
    try {
        ...
        return await emailTemplateDao.findOneAndUpdate(query, _update, {upsert: false});
    } catch (err) {
        return err;
    }
};
