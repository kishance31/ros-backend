                ...
                list: [
                    ...
                    // },
                    {
                        $project:
                        {
                            ...
                            __v: 0,
                            updatedAt: 0,
                            "purchasedLicenses._id": 0,
                            // "purchasedLicenses.licenseDetails": {
                            //     $filter: {
                            //         input: "$licenseDetails",
                            //         as: "license",
                            //         cond: {
                            //             $eq: ["$$license._id", "$purchasedLicenses.licenseId"]
                            //         }
                            //     }
                            // }
                        }
                    ...
                    // {
                    //     $addFields: {
                    //         "purchasedLicenses.licenses": {"$arrayElemAt":["$licenseDetails",]}
                    //     }
                    // },
                    // {
                    //     $group: {
                    ...
                    //         paypalDetails: {"$first": "$paypalDetails"},
                    //         randomLicenseId: {"$first": "$randomLicenseId"},
                    //         purchasedLicenses: {"$push": "$licenseDetails"}
                    //     }
                    // }
                ],
                total: [{ "$count": "count" }]
...
exports.getEmployeeByRandomId = async (randomLicenseIds, isActive) => {
    await userDao.updateMany(
        { randomLicenseId: { $in: randomLicenseIds } },
        { $set: { isActive: isActive, isFirstLogin: false } },
