...
purchaseLicenseRouter.post('/getAllPurchasedLicense/:batch/:limit', [authMiddleware.autntctAdminTkn, purchaseLicenseValidator.validatePagination], (req, res) => {
    const {
        tokenPayload: {
            userId,
        },
    ...
    purchaseLicenseFacade.getAllPurchasedLicenses({ adminId: userId, batch, limit })
        .then(result =>
            responseHandler.sendSuccess(res, result)
        )
        .catch(err => {
...
purchaseLicenseRouter.post('/activatePurchaseLicense', [authMiddleware.autntctAdminTkn], (req, res) => {
    let {
        _id,
        isActive
    } = req.body;

    purchaseLicenseFacade.activatePurchaseLicense({_id, isActive})
        .then(result =>
            responseHandler.sendSuccess(res, result)
        )
        .catch(err => {
