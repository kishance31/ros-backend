...
purchaseLicenseRouter.get('/purchase/:corporateId', [purchaseLicenseValidator.validateCorporateId, authMiddleware.autntctTkn], (req, res) => {
    const {
        userId,
        role
    } = req.tokenPayload;

    purchaseLicenseFacade.getPurchasedLicenses({ corporateId: userId, role })
        .then(result =>
            responseHandler.sendSuccess(res, result)
        )
        .catch(err => {
