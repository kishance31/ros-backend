...
const CostSummaryDao = new BaseDao(CostSummaryModel);
const productsDao = new BaseDao(productModel);
...
async function deactivateEmployee(data, _update) {
    try {
        let query = {};
        query._id = mongoose.Types.ObjectId(data._id);
        console.log(_update)
...
async function setPassword(data, projection) {
    try {
        ...
        let update = {
            $set: { password: data.newPassword, isFirstLogin: false, isActive: true },
        };
        const options = {
...
async function getProductsDetails(productIds) {
    return await productsDao.find({_id: {$in: [...productIds]}});
}
