...
async function checkEmployeeDetails(EmployeeDetails, employeeExists) {
    ...
    try {
        ...
        if (!(employeeExists.length && employeeExists[0].licenseId.toString() === EmployeeDetails.licenseId) || (employeeExists[0] && employeeExists[0].isDeleted)) {
...
async function employeeAdd(EmployeeDetails) {
    ...
    try {
        ...
        EmployeeDetails.address = JSON.parse(EmployeeDetails.address) || [];
...
async function loginEmployee(loginDetails) {
    try {
        ...
        if (!(userDetails.isActive && userDetails.isFirstLogin) || userDetails.isDeteted) {
            return employeeMapper.responseMapping(employeeConstants.CODE.Unauthorized, employeeConstants.MESSAGES.permissionDenied);
        }
