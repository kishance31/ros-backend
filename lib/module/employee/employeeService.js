...
async function setPassword(data) {
    try {
        ...
        const userDetails = await employeeDao.employeeById({ _id: data.userId, email: data.email, role: "EMPLOYEE" });
        if (!userDetails) {
            return employeeMapper.responseMapping(employeeConstants.CODE.Unauthorized, employeeConstants.MESSAGES.employeeNotFound);
        }
        if (userDetails.status != "APPROVED") {
        ...
        if(result) {
            ...
            if(userDetails.isFirstLogin) {
                return employeeMapper.loginMapping(result, data.tokens, employeeConstants.MESSAGES.setPasswordSuccess);
            } else {
