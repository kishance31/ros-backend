...
const { licenseExists } = require('../licenses/licensesMapper');
...
async function checkEmployeeDetails(EmployeeDetails, employeeExists) {
    ...
    try {
        let randomLicenseId = "", licenseExists = null;
        ...
        logger.warn(error);
        if (licenseExists && licenseExists.responseCode === 201) {
...
async function placeOrder(orderDetails) {
    ...
    try {
        ...
        return employeeDao.saveEmployeeOrderDetails(orderDetails).then((data) => {
            let Intro = "Your Order is successfully placed.";
            const html = mailHandler.mailGenHTML({
                name: isEmployeeExist[0].firstName + " " + isEmployeeExist[0].lastName,
                intro: Intro,
            });

            // send mail
            mailHandler.sendMail({
                to: isEmployeeExist[0].email,
                subject: "Order Place Successfull",
                html,
            });
