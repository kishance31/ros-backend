...
async function updateContactUsQuery(id, contactUsData) {
    try {
        return cmsDao.updateContactUsQuery(id, contactUsData).then(async (data) => {
            let mailTemplate = await mailHandler.getMailTemplate("REPLY_CONTACT_US_QUERY");
            let Intro = `${mailTemplate.description} <br/><p>${data.repliedMessage}</p>`
...
async function socialMediaLinks(socialLinkData) {
    try {
        return cmsDao.saveSocialLink(socialLinkData)
            .then((data) => {
                return cmsMapper.responseMappingData(cmsConstants.CODE.Success, "SUCCESS", data)
            }).catch((err) => {
                return cmsMapper.responseMapping(cmsConstants.CODE.INTRNLSRVR, cmsConstants.MESSAGES.internalServerError)
            })
    } catch (err) {
...
async function getSocialMediaLinks(...args) {
    try {
        return cmsDao.getSocialMediaLinks(...args)
            .then((data) => {
                return cmsMapper.responseMappingData(cmsConstants.CODE.Success, "SUCCESS", data)
            }).catch((err) => {
                return cmsMapper.responseMapping(cmsConstants.CODE.INTRNLSRVR, cmsConstants.MESSAGES.internalServerError)
