...
exports.addRole = async ({ adminRoleDetails, tokenPayload }) => {
    try {
        ...
        const result = await adminRoleDao.addRole({ ...adminRoleDetails, adminId: tokenPayload.userId });
        if (result) {
...
exports.updateRole = async (data, userId) => {
    try {
        const adminExists = await adminRoleDao.checkAdmin(userId);
        if (!adminExists) {
            return adminRoleMapper.responseMapping(adminRoleConstant.CODE.Unauthorized, adminRoleConstant.MESSAGES.invalidUser);
        }
        ...
        const result = await adminRoleDao.updateRole({ ...data, adminId: userId }, options);
        if (result) {
...
exports.deleteRole = async (data, userId) => {
    try {
        ...
        const isRoleAssigned = await adminRoleDao.checkRoleAssigned(data.roleId)
        if(isRoleAssigned) {
            return adminRoleMapper.responseMapping(adminRoleConstant.CODE.Unauthorized, adminRoleConstant.MESSAGES.roleAssigned);
        }
...
exports.getAdminForms = async() => {
    let result = await adminRoleDao.getAdminForms();
    if(result) {
        return adminRoleMapper.responseMappingData(adminRoleConstant.CODE.Success, "Permission Forms List.", result);
    }
}