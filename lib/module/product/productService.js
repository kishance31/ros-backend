...
async function productAdd(productDetails) {
	...
	try {
		...
		let productFirstThreemonthCost = (parseFloat(productDetails.product_cost) / 12) * 3;
		productDetails.first_three_month_cost = productFirstThreemonthCost;
		if (productImage) {
			productDetails.product_image = await streamUploadToCloudinary(productImage.data);
			return productDao
...
function productList(prodDetails, batch, limit) {
	return productDao
		.productList(prodDetails, batch, limit)
		.then((data) => {
...
async function productEditById(id, data) {
	try {
		let productFirstThreemonthCost = (parseFloat(data.product_cost) / 12) * 3;
		data.first_three_month_cost = productFirstThreemonthCost;
		if(typeof data.product_image !== "string") {
			data.product_image = await streamUploadToCloudinary(data.product_image.data);
		}
		console.log(data);
		return productDao
