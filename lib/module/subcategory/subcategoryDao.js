...
async function getSubCategoryByCategoryId(_query) {
    return await subCategoryDao.find(_query);
}
...
async function updateSubCategory(data, categoryData) {
    try {
        let query = {};
        query._id = data._id;

        let subCategoryRoute = "/" + categoryData.subcategory_name.trim().toLowerCase().split(" ").join("-")
        let _update = { $set: { subcategory_name: categoryData.subcategory_name, subCategoryRoute } }
