...
exports.getTotalLicenseCount = async (data) => {
    try {
        ...
        if(result) {
            let finalResult = result[0].licenses.map(lic => {
                let licName = result[0].licenseDetails.find(l => l._id.toString() === lic._id.toString());
                return {
                    licenseName: licName.type,
                    count: lic.count,
                }
            })
...
exports.decrementLicenseCount = async (data, checkUser) => {
    try {
        ...
        if(result.ok) {
            if(result.nModified) {
                return licenseMapper.decrementLicenseCount(result.randomLicenseId);
            } else {
