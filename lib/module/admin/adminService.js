...
async function createdefaultAdmin() {
	try {
		...
		const roleExist = await adminDao.checkSuperAdminRoleExist();
		let roleDetails = null;
		if (!roleExist) {
			roleDetails = await adminDao.createSuperAdminRole(config.cfg.DEFAULT_SUPER_ADMIN)
			if(exist && roleDetails) {
				await adminDao.updateUser(exist._id, {roleName: mongoose.Types.ObjectId(roleDetails._id)});
			}
		}
		if (exist) {
			return adminMapper.userExist();
		}
		let adminUser = { ...config.cfg.DEFAULT_ADMIN_USER, roleName:  mongoose.Types.ObjectId(roleDetails._id)}
		const data = await adminDao.createDefaultAdminUser(adminUser);
		await createDefaultAdminForms();
		await createDefaultCostSummary();
		await defaultContactUs();
...
async function createDefaultCostSummary() {
	await adminDao.createDefaultCostSUmmary({ ...config.cfg.COST_SUMMARY });
}
async function defaultContactUs() {
	await adminDao.defaultContactUs({ ...config.cfg.CONTACT_US_DATA });
}
async function createDefaultEmailTemplates() {
	await adminDao.createDefaultEmailTemplates(config.cfg.DEFAULT_EMAIL_TEMPLATES);
}
...
async function updateCorporateAdminStatus(id, _corporateUserId, _status) {
	try {
		...
		if (result) {
			...
			if (_status === "APPROVED") {
				mailTemplate = await mailHandler.getMailTemplate("CORPORATE_APPROVE")
				Intro = mailTemplate.description;
			} if (_status === "REJECTED") {
				mailTemplate = await mailHandler.getMailTemplate("CORPORATE_REJECT")
				Intro = mailTemplate.description;
			}
...
async function adminList(id, batch, limit) {
	try {
		...
		return await adminDao.adminList(query, batch, limit).then((data) => {
			return adminMapper.responseMappingList(adminConstants.CODE.Success, adminConstants.MESSAGES.UserListed, data)
		}).catch((err) => {
			console.log({ err })
	...
	try {
		...
		return adminDao
			...
			.then((data) => {
				...
				// let mailTemplate = await mailHandler.getMailTemplate("ORDER_DISPATCH_STATUS");
...
const forgotPassword = async (email) => {
	try {
		...
		let mailTemplate = await mailHandler.getMailTemplate("ADMIN_FORGOT_PASSWORD");
		...
		const html = mailHandler.mailGenHTML({
			...
			action: {
				...
				button: {
					...
					link: `${config.cfg.RESET_CLIENT_URL}?reset=${user.resetPasswordToken}`
				}
			}
		});
...
const resetPassword = async ({ resetToken, newPassword }) => {
	try {
		...
		let mailTemplate = await mailHandler.getMailTemplate("ADMIN_RESET_PASSWORD");
...
const changePassword = async ({ password, newPassword, email }) => {
	try {
		// find user in db
		const user = await adminDao.checkIfExist({ email });
		if (!user) {
		...
		const valid = await appUtil.verifyPassword({ password }, user);
		if (!valid) {
		...
		// send response to user
		return adminMapper.responseMapping(adminConstants.CODE.Success, adminConstants.MESSAGES.changePasswordSuccess);
