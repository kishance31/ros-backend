...
async function updateUser(userId, updateUserObj, adminId) {
    try {
        const exist = await adminDao.checkIfExist({ id: adminId });
        if (!exist) {
            return adminMapper.responseMapping(adminConstants.CODE.Unauthorized, adminConstants.MESSAGES.UserNotExist);
        }
        const options = {
        ...
        return adminDao.updateUser(userId, updateUserObj, options).then((data) => {
...
async function getCorporateUsers(userId) {
    try {
        const exist = await adminDao.checkIfExist({ id: userId });
        if (!exist) {
            return adminMapper.responseMapping(adminConstants.CODE.Unauthorized, adminConstants.MESSAGES.UserNotExist);
        ...
        let result = await adminDao.getCorporateUsers();
        if (result) {
            return adminMapper.responseMappingAdmins(adminConstants.CODE.Success, adminConstants.MESSAGES.corporateListSuccess, result);
        }
