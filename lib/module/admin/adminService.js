...
async function updateCorporateAdminStatus(id, _corporateUserId, _status) {
	try {
		...
		if (result) {
			...
			let Intro =""
			if (_status==="APPROVED"){
				 Intro = adminConstants.MAIL_MESSAGE.STATUS_CHANGED_APPROVED
			} if (_status === "REJECTED") {
...
async function getCorporateUsers(userId) {
	try {
		...
		if (result) {
			return adminMapper.responseMappingData(adminConstants.CODE.Success, adminConstants.MESSAGES.corporateListSuccess, result);
		}
